...
$(document).ready(function () {
    ...
    // Function to handle sticky header and scroll effects
    function headerStyle() {
        if ($('.header').length) {
            var windowpos = $(window).scrollTop(); // Get the current scroll position
            var siteHeader = $('.header');
            
            // Check scroll position and add/remove the 'fixed-header' class
            if (windowpos >= 250) {
                siteHeader.addClass('fixed-header'); // Make header sticky and change background
            } else {
                siteHeader.removeClass('fixed-header'); // Reset to transparent background
            }
        }
    }
    ...
    // Run on page load (to apply the style if already scrolled)
    headerStyle();
